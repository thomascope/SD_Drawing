...
for i = 1:length(scan_paths)
    oldfilenames{i}= cellstr(['/' fullfile(split_stem{crun}{1:end-1}) '/wc1' split_stem{i}{end}]);
    newfilenames{i}= cellstr(['/' fullfile(split_stem{crun}{1:end-1}) '/s_0_wc1' split_stem{i}{end}]);
    try
...
newfilenames = cell(length(scan_paths),1);
for i = 1:length(scan_paths)
    oldfilenames{i}= cellstr(['/' fullfile(split_stem{crun}{1:end-1}) '/swc1' split_stem{i}{end}]);
    newfilenames{i}= cellstr(['/' fullfile(split_stem{crun}{1:end-1}) '/s_8_wc1' split_stem{i}{end}]);
    try
        movefile(char(oldfilenames{i}),char(newfilenames{i}));
    catch
    end
